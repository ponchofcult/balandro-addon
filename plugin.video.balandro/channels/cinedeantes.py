...
def mainlist_pelis(item):
    ...
    itemlist.append(item.clone ( title = ' - Buscar película ...', action = 'search', grupo = 'alfab', search_type = 'movie' ))
    itemlist.append(item.clone ( title = ' - Buscar agrupación ...', action = 'search', grupo = 'agrupa', search_type = 'movie' ))
...
def list_letra(item):
    ...
    if itemlist:
        if len(itemlist) == 20:
            itemlist.append(item.clone( title = '>> Página siguiente', desde = hasta, action = 'list_letra',
                                        filtro_search = item.filtro_search, text_color = 'coral' ))

        elif item.filtro_search.upper() == 'L':
            if title == 'Madame Curie':
                itemlist.append(item.clone( title = '>> Página siguiente', desde = 0,
    ...
    if not i == 0:
       ...
       if itemlist:
          if i > perpage:
             if num_matches > hasta:
                next_page = item.page + 1
                itemlist.append(item.clone( title = '>> Página siguiente', page = next_page, filtro_search = item.filtro_search,
