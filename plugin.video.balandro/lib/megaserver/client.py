...
class Client(object):
    VIDEO_EXTS = {'.avi': 'video/x-msvideo', '.mp4': 'video/mp4', '.mkv': 'video/x-matroska', '.m4v': 'video/mp4', 
       '.mov': 'video/quicktime', '.mpg': 'video/mpeg', '.ogv': 'video/ogg', '.ogg': 'video/ogg', 
    ...
    def get_play_list(self):
        if len(self.files) > 1:
            ...
            return 'http://' + self.ip + ':' + str(self.port) + '/' + quote(self.files[0].name.encode('utf8'))
    ...
    def get_files(self):
        ...
        if self.files:
            for file in self.files:
                ...
                u = 'http://' + self.ip + ':' + str(self.port) + '/' + quote(n)
                s = file.size
    ...
    def add_url(self, url):
        ...
        if url.startswith('F!'):
            if len(url.split('!')) == 3:
                ...
                raise Exception('Enlace no v\xc3\xa1lido')
    ...
    def api_req(self, req, get=''):
        seqno = random.randint(0, 4294967295L)
    ...
    def aes_cbc_decrypt(self, data, key):
        try:
            ...
            try:
                ...
                logger.info("Mega: falta criptografía en este sistema")
    ...
    def dec_attr(self, attr, key):
        attr = self.aes_cbc_decrypt(attr, self.a32_to_str(key)).rstrip('\x00')
